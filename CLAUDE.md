...
- `boot.sh` は `claude --channels plugin:telegram@claude-plugins-official -c` を無限ループで実行し、終了したら `CLAUDE_RESTART_DELAY` 秒待って再起動する。
...
- `CLAUDE_SESSION_ID` がある場合は `--resume <id>`、ない場合は `-c` で前回セッションを継続する。
...
- Bot 作成は BotFather で行う。token は `/telegram:configure <token>` または `TELEGRAM_BOT_TOKEN=... scripts/configure-telegram.sh` で `~/.claude/channels/telegram/.env` に保存するのが基本。
